...
if not re.fullmatch(re.escape(version)+r'-[0-9TZ]+-[0-9]+',rid): sys.exit('Invalid run ID')
run=root/'logs'/rid
...
if ok:
    ms=sum(r[1] for r in good); flops=sum(2*(h-kh+1)*(w-kw+1)*kh*kw for h,w,kh,kw in cases)
    cells+=['%.2f / %.4f'%(r[1],r[2]) for r in good]+['%.2f'%ms,'%.4f'%(flops/ms/1e6),status]
else: cells+=['—']*6+[status]
line='| '+' | '.join(cells)+' |'
table=root/'scores.md'
header='# CONV 成绩表\n\n自测；非官方榜单分数。每项：ms / GFLOPS。GCC -O3，38 线程，NUMA 1，OMP_PROC_BIND=true，每项计时 1 次。\n总 GFLOPS 按总 FLOP / 总时间计算；失败/未完成不填写性能。\n\n| Version | Run | SHA256 (12) | Case1 | Case2 | Case3 | Case4 | Total(ms) | GFLOPS | Status |\n|---|---|---|---|---|---|---|---:|---:|---|\n'
